...
hook OnGameModeInit()
{
	...
	{
		print("[MAP LOADEDER ERROR] Directory '"DIRECTORY_SCRIPTFILES"' not found. Creating directory.");
		dir_create(DIRECTORY_SCRIPTFILES);
	}

	if(!dir_exists(DIRECTORY_SCRIPTFILES DIRECTORY_MAPS))
	{
		print("[MAP LOADEDER ERROR] Directory '"DIRECTORY_SCRIPTFILES DIRECTORY_MAPS"' not found. Creating directory.");
		dir_create(DIRECTORY_SCRIPTFILES DIRECTORY_MAPS);
	}

	if(!dir_exists(DIRECTORY_SCRIPTFILES DIRECTORY_MAPS DIRECTORY_SESSION))
	{
		print("[MAP LOADEDER ERROR] Directory '"DIRECTORY_SCRIPTFILES DIRECTORY_MAPS DIRECTORY_SESSION"' not found. Creating directory.");
	...
	if(gDebugLevel > DEBUG_LEVEL_NONE)
		printf("[MAP LOADEDER INFO] [Init] Debug Level: %d", gDebugLevel);
	...
	{
		printf("[MAP LOADEDER INFO] [Init] %d Total objects", gTotalLoadedObjects);
		printf("[MAP LOADEDER INFO] [Init] %d Objects to remove", gTotalObjectsToRemove);
	}
...
LoadConfig()
{
	...
	if(file)
	{
		...
		for(new i; i < len; i++)
		{
			...
			printf("[MAP LOADEDER ERROR] Unknown option character at column %d.", i);
...
LoadMapsFromFolder(folder[])
{
	...
	if(gDebugLevel >= DEBUG_LEVEL_FOLDERS)
	{
		...
		printf("[MAP LOADEDER DEBUG] [LoadMapsFromFolder] Reading directory '%s': %d files, %d .map files, %d folders", foldername, totalfiles, totalmapfiles, totalfolders);
	}
	...
	if(gDebugLevel >= DEBUG_LEVEL_FOLDERS)
		print("[MAP LOADEDER DEBUG] [LoadMapsFromFolder] Finished reading directory.");
}
...
LoadMap(filename[])
{
	...
	if(!fexist(filename))
	{
		printf("[MAP LOADEDER ERROR] file: \"%s\" NOT FOUND", filename);
		return 0;
	}
	...
	if(!file)
	{
		printf("[MAP LOADEDER ERROR] file: \"%s\" NOT LOADED", filename);
		return 0;
	}
	...
	if(gDebugLevel >= DEBUG_LEVEL_FILES)
	{
		...
		printf("\n[MAP LOADEDER DEBUG] [LoadMap] Reading file '%s': %d lines.", filename, totallines);
	}
	...
	while(fread(file, line))
	{
		...
		if(!strcmp(funcname, "options", false))
		{
			if(!sscanf(funcargs, "p<,>ddf", globalworld, globalinterior, globalrange))
			{
				if(gDebugLevel >= DEBUG_LEVEL_DATA)
					printf(" [MAP LOADEDER DEBUG] [LoadMap] Updated options to: %d, %d, %f", globalworld, globalinterior, globalrange);
		...
		if(!strcmp(funcname, "Create", false, 6)) // Scan for any function starting with 'Create', this covers CreateObject, CreateDynamicObject, CreateStreamedObject, etc.
		{
			if(!sscanf(funcargs, "p<,>dffffffD(-1)D(-1){D(-1)}F(-1.0)", modelid, posx, posy, posz, rotx, roty, rotz, world, interior, range))
			{
				...
				if(gDebugLevel == DEBUG_LEVEL_DATA)
				{
					printf(" [MAP LOADEDER DEBUG] [LoadMap] Object: %d, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f (%d, %d, %f)",
		...
		if(!strcmp(funcname, "SetObjectMaterialText"))
		{
			if(!sscanf(funcargs, "p<,>{s[16]} p<\">{s[1]}s[128]p<,>{s[1]} d s[32] p<\">{s[1]}s[32]p<,>{s[1]} ddxxd", tmpObjText, tmpObjIdx, tmpObjResName, tmpObjFont, tmpObjFontSize, tmpObjBold, tmpObjFontCol, tmpObjBackCol, tmpObjAlign))
			{
				if(gDebugLevel == DEBUG_LEVEL_DATA)
				{
					printf(" [MAP LOADEDER DEBUG] [LoadMap] Object Text: '%s', %d, '%s', '%s', %d, %d, %x, %x, %d",
		...
		if(!strcmp(funcname, "SetObjectMaterial") || !strcmp(funcname, "SetDynamicObjectMaterial"))
		{
			if(!sscanf(funcargs, "p<,>{s[16]}dd p<\">{s[1]}s[32]p<,>{s[1]} p<\">{s[1]}s[32]p<,>{s[1]} x", tmpObjIdx, tmpObjMod, tmpObjTxd, tmpObjTex, tmpObjMatCol))
			{
				if(gDebugLevel == DEBUG_LEVEL_DATA)
				{
					printf(" [MAP LOADEDER DEBUG] [LoadMap] Object Material: %d, %d, '%s', '%s', %x",
		...
		if(!strcmp(funcname, "RemoveBuildingForPlayer"))
		{
			if(gTotalObjectsToRemove < MAX_REMOVED_OBJECTS)
			{
				if(!sscanf(funcargs, "p<,>{s[16]}dffff", modelid, posx, posy, posz, range))
				{
					if(gDebugLevel == DEBUG_LEVEL_DATA)
					{
						printf(" [MAP LOADEDER DEBUG] [LoadMap] Removal: %d, %.2f, %.2f, %.2f, %.2f",
			...
			{
				printf(" [MAP LOADEDER ERROR] [LoadMap] Removal on line %d failed. Removal limit reached.", linenumber);
			}
		}
	...
	if(gDebugLevel >= DEBUG_LEVEL_FILES)
		printf("[MAP LOADEDER DEBUG] [LoadMap] Finished reading file. %d objects loaded from %d lines, %d total operations.", objects, linenumber, operations);
...
hook OnPlayerDisconnect(playerid, reason)
{
	...
	if(gDebugLevel >= DEBUG_LEVEL_INFO)
		printf("[MAP LOADEDER INFO] [OnPlayerDisconnect] Removing session data file for %s", name);
...
RemoveObjects_FirstLoad(playerid)
{
	...
	if(!file)
		printf("[MAP LOADEDER ERROR] [RemoveObjects_FirstLoad] Opening file '%s' for write.", filename);

	if(gDebugLevel >= DEBUG_LEVEL_INFO)
		printf("[MAP LOADEDER INFO] [RemoveObjects_FirstLoad] Created session data for %s", name);
	...
	for(new i; i < gTotalObjectsToRemove; i++)
	{
		...
		if(gDebugLevel >= DEBUG_LEVEL_DATA)
			printf("[MAP LOADEDER INFO] [RemoveObjects_FirstLoad] Write: [%x.%x.%x.%x.%x]", buffer[0], buffer[1], buffer[2], buffer[3], buffer[4]);
...
RemoveObjects_OnLoad(playerid)
{
	...
	{
		if(gDebugLevel >= DEBUG_LEVEL_INFO)
			printf("[MAP LOADEDER INFO] [RemoveObjects_OnLoad] Session data for %s doesn't exist, running firstload.", name);
	...
	if(gDebugLevel >= DEBUG_LEVEL_INFO)
		printf("[MAP LOADEDER INFO] [RemoveObjects_OnLoad] Loading removals for %s", name);
	...
	for(new i; i < gTotalObjectsToRemove; i++)
	{
		...
		{
			if(gDebugLevel == DEBUG_LEVEL_DATA)
				printf(" [MAP LOADEDER DEBUG] [RemoveObjects_OnLoad] Skipping object removal %d (model: %d)", i, gModelRemoveData[i][remove_Model]);

			continue;
		}

		if(gDebugLevel == DEBUG_LEVEL_DATA)
			printf(" [MAP LOADEDER DEBUG] [RemoveObjects_OnLoad] Removing object %d (model: %d)", i, gModelRemoveData[i][remove_Model]);
		...
		if(gDebugLevel >= DEBUG_LEVEL_DATA)
			printf("[MAP LOADEDER INFO] [RemoveObjects_OnLoad] Append: [%x.%x.%x.%x.%x]", buffer[0], buffer[1], buffer[2], buffer[3], buffer[4]);
